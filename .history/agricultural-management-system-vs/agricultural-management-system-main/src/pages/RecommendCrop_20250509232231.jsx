...
const RecommendCrop = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const filtered = crops.filter((crop) => {
      let cost = crop.TotalCost;
      if (!cost && crop.Stages && crop.Stages.length > 0) {
        cost = crop.Stages.reduce((sum, stage) => sum + (stage.TotalCost || 0), 0);
      }
      cost = cost || 0;
        ...
        <div style={{ marginTop: '2rem' }}>
          ...
          <ul>
            {recommendedCrops.map((crop) => {
              ...
              return (
                <li key={crop.Id}>
                  <strong>{crop.CropName}</strong> - Estimated Cost: {cost}
                </li>
              );
            })}
          </ul>
